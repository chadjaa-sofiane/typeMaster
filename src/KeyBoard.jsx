...
import SpecialKeysIcons from "./SpecialKeysIcons.jsx"

const KeyBoard = () => {
  const [rows, setRows] = useState(
    window.keyboard_layouts.ISO["alpha_numerics"]
  );
  const [rows_alt, setRowsAlt] = useState(
    window.keyboard_layouts.ISO["alpha_numerics_alt"]
  );
  ...
  useEffect(() => {
    ...
    setRowsAlt((rows_alt) =>
      rows_alt.concat([window.keyboard_layouts.ISO["last_row"]])
    );
  ...
  if (loading) {
    ...
    return (
      <>
        <SpecialKeysIcons specialKey="ShiftRight" />
        {shiftIsDown ? (
          <div
            className="keyboard"
